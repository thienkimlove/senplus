...
<div>
    <table border="1">
        ...
        <tr>
            ...
            <th>Nhu cầu thay đổi về tiêu chí thành công</th>
            <th>Chênh lệch</th>
        </tr>
        </thead>
        <tbody>
...
<div>
    Tiêu chí thành công hiện tại {{ \App\Helpers::getMatchName($explain['extends'][6]['percentMatch']) }}({{$explain['extends'][6]['percentMatch']}}%)

    @if (count($explain['extends'][6]['bigThan']) == 0) do cả 4 giá trị cột chênh lệch đều nhỏ hơn 5. @endif

    @if (count($explain['extends'][6]['bigThan']) == 3) do có 3 giá trị của cột chênh lệch lớn hơn hoặc = 5, tương ứng với
    ...
    @if (count($explain['extends'][6]['bigThan']) == 2) do có 2 giá trị của cột chênh lệch lớn hơn hoặc = 5, tương ứng với
    @foreach ($explain['extends'][6]['bigThan'] as $option)
        {{ $explain['all']->where('option', $option)->first()->ten_van_hoa }} ({{ \App\Helpers::getXValue($explain, 6, $option) }} điểm),
    @endforeach
    @endif

    @if (count($explain['extends'][6]['bigThan']) == 1) do có 1 giá trị của cột chênh lệch lớn hơn hoặc = 5, tương ứng với
    @foreach ($explain['extends'][6]['bigThan'] as $option)
        {{ $explain['all']->where('option', $option)->first()->ten_van_hoa }} ({{ \App\Helpers::getXValue($explain, 6, $option) }} điểm),
    @endforeach
    @endif

    @if (count($explain['extends'][6]['bigThan']) == 4) do cả 4 giá trị của cột chênh lệch đều lớn hơn hoặc = 5. @endif
